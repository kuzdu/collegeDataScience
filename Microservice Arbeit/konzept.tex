...
\begin{itemize}
	...
	\item welche \textbf{Erweiterungen} entstehen durch eine API.
\end{itemize}
...
Ein Beispiel für eine User Story könnte sein: Als Nutzer (\textit{wer}) möchte ich mein Spielprofil teilen (\textbf{was}), um mich besser mit meinen Freunden messen zu können ({\textbf{warum}).\\
...
\textit{Spielprinzip Stirnraten: Die Spieleranzahl muss mindestens zwei betragen. Ein Spieler wählt aus verschiedenen Kategorien eine aus und hält sich das Telefon an die Stirn. Es erscheint ein Begriff, welchen der Gegenüber erklären muss. Errät der Spieler den Begriff, neigt er das Telefon nach vorne und ein neuer Begriff erscheint. Weiß er ihn nicht, kann er diesen überspringen, in dem er das Telefon nach hinten neigt. Ziel ist es, innerhalb einer frei wählbaren Zeit (z.B. 60 Sekunden), so viele Begriffe wie möglich zu erraten.}

\subsubsection{Erfassung Stirnratens IST-Stand}

In der folgenden Tabelle wird gezeigt, welche Funktionen die App bereits auf dem Gerät bereitstellt, welche aber zukünftig serverseitig erledigt werden sollen. 
...
\begin{itemize}
	\item Als Betreiber möchte ich neue Wörter über eine Schnittstelle hinzufügen, editieren und löschen können, um die Datenbank schneller und leichter pflegen zu können
	\item Als Betreiber möchte ich eine Datenbank, um nicht für zwei Apps (iOS und Android) den Datenbestand zu pflegen
	\item Als Betreiber möchte ich entscheiden können, in welcher Sprache (englisch oder deutsch) ich Wörter manipuliere, um sinnvolle Daten zu gewährleisten
	...
	\item Als Nutzer möchte automatisch die Sprache angezeigt kriegen, welche für mich relevant ist, weil es mir sonst zu kompliziert ist
\end{itemize}
...
\begin{itemize}
	...
	\item Als Betreiber möchte ich Bilder pro Kategorien hinzufügen, editieren und löschen können, um ein ansprechendes Bild zu hinterlegen
	\item Als Betreiber möchte ich eine Kategorie als Premium kennzeichnen können, um die Premiumversion attraktiver zu gestalten
	...
	\item Als Betreiber möchte ich die Rangliste manipulieren können, um missbrauch zu verhindern.
	\item Als Nutzer möchte ich mir andere Nutzer und deren Statistik ansehen, um mich zu vergleichen.
	\item Als Betreiber möchte ich die Kategorien sortieren können, um die Anordnung für die Nutzer bestmöglich zu gestalten.  
	\item Als Nutzer möchte ich eigene Wörter einreichen können, weil mir häufiger Wörter im Spiel fehlen
	\item Als Betreiber möchte ich eingereichte Wörter zulassen oder ablehnen können, um die Datenqualität zu gewährleisten oder zu vergrößeren. 
...
\begin{itemize}
	...
	\item Als Betreiber möchte ich Themes und Farbcodes online bereitstellen, um schneller und leichter Themes bereitzustellen 

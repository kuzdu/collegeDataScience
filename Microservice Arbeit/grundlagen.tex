...
Es emphielt sich die Autorisierung in den Microservices zu überprüfen, da diese den entsprechenden Datenbestand haben. Um unnötige Last zu verhindern, kann die Validität - ob es sich überhaupt um einen gültigen Request handelt - bereits im API-Gateway überprüft werden. Die Authentifizierung sollte in einen eigenen Service oder ins API-Gateway verlagert werden.\cite{rewe2019mic_ppp}\cite{richardson2019mic_pattern} Wichtig ist, dass die Authentifizierung an einer Stelle durchgeführt wird, um fehlerhafte Implementierungen zu verhindern. 
...
Die Idee ist, dass der Benutzer mittels Login ein Security-Token erhält, welches verwendet wird, um sensible Anfragen zu verifizieren. \\

%HIER die Tokenbeschaffenheit erklären JWT vs. opaque

Für Microservices empfiehlt sich eine sogenannte OAuth2 Implementierung, welche der Variante B) angelehnt ist.\cite{richardson2019mic_pattern} OAuth2 ist ein Sicherheitsstandard, welches Autorisierungen zwischen verschiedene Anwendungen erlaubt. Ursprünglich wurde das Authentifizierungsprotokoll so desigend, dass Drittanwendungen Zugang zu Informationen erhalten, ohne dass Passwörter weitergeleitet werden müssen.\cite{richardson2019mic_pattern}  Beispielsweise wird OAuth2 verwendet, wenn Benutzer sich über ihren Facebook-Account bei Drittplattformen anmelden. Die Drittplattformen können natürlich nicht das Facebook-Passwort einsehen, erhalten aber je Anwendungsfall Zugriff auf verschiedene Ressourcen (z.B. Lesezugriff auf E-Mail-Adresse und/oder Konakte, Schreibzugriffe zum Teilen von Nachrichten usw.). \\
...
%TODO: Ab Seite 356 gehts weiter
%TODO: Bild einbinden auth_request, verlinken auf richardson und kurz erläutern
%Worauf achten bei der Authorisierung: Benutzerdetails, man darf nur seine eigenen abrufen und ein Agent darf alle abrufen
%TODO: Wie kann man das regeln: Durch JWT Token 



Im Gegensatz zum Monolithen muss jeder Microservice die Autorisierung abfragen. 

Um bei den Anforderungen aus 3.1 zu bleiben, sollte ein Microservices zustandslos sein, damit er repliziert, skaliert usw. werden kann. 

1. Distributed Session Management
Welche Methodiken gibt es: 
- Sticky Sessions: alles geht über einen Knoten (Load Balancer, der es kontrolliert) (wenn umziehen gehen die iwie verloren)
- Session Replication: Jede Instanz speichert die Session, welche sich synchronisieren (kostet immer mehr Bandbreite)
- Zentralisierter Session Speicher 

2. Token 
Speicher ist anders: Token wird beim User gehalten, Sessions auf dem Server
Token müssen fälschungssicher sein

Im folgenden Abschnitt werden gängige Authentifizierungsmöglichkeiten vorgestellt. 
Authenfizierungsmöglichkeiten können 
JWT Token: JWT (Json Web Token) is an open standard (RFC 7519) 

3. Single Sign On 

4. API Gateway + Client Token 

5. Third Party Application 
1. Api Token generieren z.B. in Git
2. OAuth (Facebook, Google)
3. 
...
Tieferes Verständnis von Pattern -> siehe Chris Richardson Microservice Pattern
...
% https://www.youtube.com/watch?v=g52hubmhUP4

% Unterscheidung Authentifizierung (wer ist man) und Authorisierung (was darf man)
